-- Elixhauser comorbidity flags and van Walraven score for all OPSCC cohort patients
-- Lookback: 12 months before first_hnc_date (exclusive), across all three claim sources
-- Codes: Quan et al. ICD-10-CM adaptation (see comorbidity.py)
-- solidtum/metacanc reflect prior cancers only; index HNC codes excluded by lookback window

DROP TABLE IF EXISTS opscc_comorbidity;

CREATE TABLE opscc_comorbidity AS
...
WITH dx_raw AS (
    ...
    JOIN opscc_cohort h ON i.DSYSRTKY = h.DSYSRTKY,
    UNNEST([
    ...
    WHERE code IS NOT NULL
      AND TRY_STRPTIME(i.THRU_DT, '%Y%m%d')
              BETWEEN h.first_hnc_date - INTERVAL 12 MONTH
                  AND h.first_hnc_date - INTERVAL 1 DAY
    ...
    JOIN opscc_cohort h ON c.DSYSRTKY = h.DSYSRTKY
    ...
    JOIN opscc_cohort h ON oc.DSYSRTKY = h.DSYSRTKY,
    UNNEST([
        oc.PRNCPAL_DGNS_CD,
...
flags AS (
    SELECT
        ...
        -- Reflects prior cancers only; index HNC codes are excluded by the lookback window
        MAX(CASE WHEN
