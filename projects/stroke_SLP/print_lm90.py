...
print(f"=== 90-DAY LANDMARK — HOME + HHA ONLY ===")
print(f"Total after landmark + home/HHA filter: {len(lm_slp):,}")
print(f"Group sizes: {lm_slp['grp'].value_counts().sort_index().to_dict()}")
print(f"Excluded by landmark (died day 30-90): {len(df[df['slp_outpt_any_90d']==1]) - len(lm_slp):,} fewer than 30d landmark\n")
