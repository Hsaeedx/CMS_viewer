-- Elixhauser comorbidity flags and van Walraven score
-- Adapted from: opscc/queries/hnc_elixhauser.sql
-- Changes:
--   - Join io_cohort (not hnc_universe), anchor = death_dt
--   - Lookback window: months 13-2 before death (excludes terminal/near-death period)
--   - HNC codes NOT excluded from solidtum/metacanc (IO cohort already has active HNC)
-- Output: io_comorbidity

SET memory_limit='24GB';
SET threads=12;
SET temp_directory='F:\CMS\duckdb_temp';

DROP TABLE IF EXISTS io_comorbidity;

CREATE TABLE io_comorbidity AS
...
WITH dx_raw AS (
    ...
    JOIN io_cohort h ON i.DSYSRTKY = h.DSYSRTKY,
    UNNEST([
    ...
    FROM io_car_lines c
    JOIN io_cohort h ON c.DSYSRTKY = h.DSYSRTKY
    ...
    JOIN io_cohort h ON oc.DSYSRTKY = h.DSYSRTKY,
    UNNEST([
        oc.PRNCPAL_DGNS_CD,
...
flags AS (
    SELECT
        ...
        -- Metastatic cancer (weight: 12) — NOT excluded for IO cohort (active HNC expected)
        MAX(CASE WHEN
            code LIKE 'C77%' OR code LIKE 'C78%' OR code LIKE 'C79%' OR code LIKE 'C80%'
        THEN 1 ELSE 0 END) AS metacanc,

        -- Solid tumor without metastasis (weight: 4) — NOT excluded for IO cohort
        MAX(CASE WHEN
