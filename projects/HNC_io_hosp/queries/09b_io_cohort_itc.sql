...
SELECT
    ...
    -- Curative therapy (NULL for patients without documented prior therapy)
    cur.curative_therapy_date,
    cur.primary_curative_type,
    COALESCE(cur.had_surgery,   0) AS had_surgery,
    COALESCE(cur.had_radiation, 0) AS had_radiation,
    -- ITT flag: 1 = in primary cohort, 0 = de novo / no prior curative therapy documented
    (cur.DSYSRTKY IS NOT NULL)::INT AS had_prior_curative_therapy,
